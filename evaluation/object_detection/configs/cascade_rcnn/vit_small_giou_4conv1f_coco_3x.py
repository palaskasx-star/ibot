...
model = dict(
    backbone=dict(
        _delete_=True,
        type='TimmViTWithFPN',                 
        model_name='vit_small_patch16_224',    # The timm registry name for ViT-Small
        pretrained=True,
        with_fpn=True,
        out_indices=[3, 5, 7, 11],
        drop_path_rate=0.1,                    # Keep this!
    ),
    neck=dict(
        type='FPN',
        in_channels=[384, 384, 384, 384],      # Keep this (ViT-Small outputs 384 dim)
        out_channels=256,
        num_outs=5),
    roi_head=dict(
        bbox_head=[
